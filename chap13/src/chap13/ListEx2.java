 ...
 * 			   => 요소객체가 Comparable 인터페이스 구현 클래스 격체여야함.
...
/*
 * 정렬관련 인터페이스
 * 	1. Comparable : 클래스가 구현하여, 클래스 객체의 기본 정렬방식 설정에 사용됨.
 * 				 	int compareTo(Object o) 추상메서드를 가짐.
 * 	2. Comparator : 기본정렬방식과 상관없이 실행 중 정렬방식을 설정 시 사용됨.
...
//		Collections.sort(list3,(d1,d2)->d1.value2 - d2.value2);	//람다방식
		
		//Comparator 인터페이스 객체 : FunctionalInterface임
		//FunctionalInterface 인터페이스 : 추상메서드가 한 개만 존재하는 인터페이스
		//							  : 람다방식이 가능
