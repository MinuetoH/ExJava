 ...
 * 
 * Collection 프레임워크 : 객체들의 집합구현하기 위한 클래스와 인터페이스의 모임
 * 					    java.util 패키지에 속함
 * 
 * Collection 인터페이스 : 객체들의 집합
 * 하위인터페이스
 * 	1. List 인터페이스 : 객체를 저장한 순서를 유지.
 * 					  인덱스(첨자) 사용가능. 0부터 시작
 * 		구현클래스 : ArrayList, Vector, LinkedList
 * 				 Vector : Collect 프레임워크이전에 사용되던 클래스
 * 						  이전의 메서드와 Collection 프레임워크의 메서드 모두 사용가능
...
//		list.add("a");	//Integer 객체가 아니므로 list 객체에 추가 불가
		...
		//subList(1,4) : list의 1번 인덱스부터 3번 인덱스까지의 부분리스트.
		...
		//정렬하기
		//Collection 인터페이스
		//Collection 클래스 : Collection 객체의 기능을 수행할 수 있는 클래스
		//list 객체의 요소들을 정렬하기
