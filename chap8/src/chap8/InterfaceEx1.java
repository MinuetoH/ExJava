...
//		Complexerable c1 = new complexerable();	//인터페이스는 직접 객체화 불가
		...
		if(c1 instanceof Complexerable) {
			...
			Complexerable c = c1;	//자동형변환.
			c.print();	//
			c.scan();
			c.receive("02-5678-1234");
			c.send("02-5678-1234");
			System.out.println(c.INK);	
//			System.out.println(c.ink);	//ink는 Complexerable 인터페이스의 멤버가 아님.
		}
...
//			System.out.println(p.ink);	//ink는 Printerable 인터페이스의 멤버가 아님.
		}
		if(c1 instanceof Scannerable) {
			System.out.println("c1 객체는 Scannerable 객체임");
			Scannerable s = c1;
//			s.print();	//Printerable 멤버임
			s.scan();
//			s.receive("02-5678-1234");	//Faxable 멤버임
//			s.send("02-5678-1234");		//Faxable 멤버임
//			System.out.println(s.INK);	//Printerable 멤버임
//			System.out.println(s.ink);	//ink는 Scannerable 인터페이스의 멤버가 아님.
		}
		if(c1 instanceof Faxable) {
			System.out.println("c1 객체는 Faxable 객체임");
			Faxable f = c1;
//			f.print();	//Printerable 멤버임
//			f.scan();	//Scannerable 멤버임
			f.receive("02-5678-1234");	
			f.send("02-5678-1234");		
//			System.out.println(f.INK);	//Printerable 멤버임
//			System.out.println(f.ink);	//ink는 Faxable 인터페이스의 멤버가 아님.
		}
	}
