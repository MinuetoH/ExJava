package chap8;
/*
 * Animal 클래스는 다음과 같다. 구동클래스를 실행했을 때 다음의 결과가 나오도록 프로그램 구현하기
 * 
 * [결과]
 * 비둘기는 작은 벌레를 잡아 먹는다.
 * 비둘기는 날아다니는 새입니다.
...
public class Exam1 {
	...
	public static void main(String[] args) {
		...
		for(Animal a : arr) {
			a.eat();
			if(a instanceof Flyable) {	//Dove 객체는 Flyable 형변환 가능, Monkey는 불가, Eagle은 가능.
				Flyable f = (Flyable)a;	//자동형변환 불가. 강제로 연산자 필요
//				a.fly();	//fly() 메서드 없음.
				f.fly();	// Flyable 타입의 참조변수로 호출
			}
		}
	}
