...
//		Shape s = new Shape("도형");	//추상클래스는 객체화 불가
		...
		//도형의 면적의 합과 둘레합을 출력하기
		//arr[0] : Shape 타입의 참조변수
		//arr[0].area() : Circle 객체의 area() 메서드 호출
		//arr[1].area() : Rectangle 객체의 area() 메서드 호출
		System.out.println("면적의 합:"+(arr[0].area() + arr[1].area()));
		System.out.println("면적의 합:"+(arr[0].length() + arr[1].length()));
