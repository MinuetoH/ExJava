 ...
 *   맴베 메서드는 객체의 최종 오버라이딩된 메서드가 호출된다.
 *   
 * - 객체와 참조변수의 관계
...
class AutoBike3 extends Bike3 {
	...
	void drive() {
		if(!power) power();
		System.out.println("출발버튼은 누른다");
	}
}
public class AutoBikeEx3 {
	public static void main(String[] args) {
		AutoBike3 ab = new AutoBike3(2);
		System.out.println("가격:" + ab.price);
		ab.drive();
		ab.stop();
		Bike3 b = ab;	//형변환 부모클래스 <= 자손클래스 : 형변환 연산자 생략 가능
...
//		Bike3 b2 = new Bike3(2);
		...
		//자손클래스 <= 부모클래스 형변환 시 형변환 연산자 생략 불가.
