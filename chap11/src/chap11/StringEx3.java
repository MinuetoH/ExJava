...
public class StringEx3 {
	public static void main(String[] args) {
		...
		//s.charAt(4) : s 문자열에서 4번인덱스에 해당하는 문자 한 개 리턴.
		System.out.println("s.charAt(4):"+s.charAt(4));	//E
		//s.compareTo("abc") : 정렬에서 사용되는 메서드. 대소문자 구분
		//			  결과 음수 : s 문자열이 abc 문자열보다 앞쪽
		// 			  결과 양수 : s 문자열이 AAA 문자열보다 뒷쪽
		...
		//s.equalsIgnoreCase("ABCDEFGH") : s 문자열이 대소문자 상관없이 "abcdefgh" 문자열과 같은지 여부
		...
		//s.indexOf('i',3) : 3번 인덱스부터 i 문자의 위치(인덱스) 리턴
		System.out.println("s.indexOf('i',3)" + s.indexOf('i',3));	//2
		//해당 문자가 없는 경우 -1 리턴
		System.out.println("s.indexOf('Q')" + s.indexOf('Q'));	//2
		//s.lastIndexOf('i') : 뒤쪽에서 i 문자의 위치. 
		//					   리턴 인덱스 값은 앞부터 시작.
		System.out.println("s.lastIndexOf('i')" + s.lastIndexOf('i'));	//2
		//s.length() : 문자열의 길이 리턴
		System.out.println("s.length():"+s.length());
		//s.replace("is","QR") : 문자열 치환
		System.out.println("s.replace('is','QR'):" + s.replace("is","QR"));	//ThQR QR a String
		System.out.println("s.replace('is',''):" + s.replace("is",""));		//Th  a String
		//substring : 부분문자열
		//s.substring(5) : 5번 인덱스부터 이후 부분만 리턴.
		System.out.println("s.substring(5):" + s.substring(5));	//is a String
		//s.substring(5,13) : 5번 인덱스부터 13번 인덱스 앞부분만 리턴.
		//					  5번 인덱스부터 12번 인덱스까지만 리턴.
		System.out.println("s.substring5,13):" + s.substring(5,13));	//is a Str
		//toUpperCase : 대문자 변경
